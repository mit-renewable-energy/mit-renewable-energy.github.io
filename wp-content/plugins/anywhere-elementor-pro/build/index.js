...
const DynamicBgHandler = function ($scope, $) {
    ...
    if ($scope.hasClass("elementor-element-edit-mode") && $scope.hasClass("ae-featured-bg-source-custom_field")) {
        var element = document.querySelector('.elementor-element-'+scope_id);
        var elementClasses = element.className
        var classArr = elementClasses.split(" ");
        let fieldClass = classArr.filter(clas =>  clas.startsWith('ae-feature-bg-custom-field-'));
        if(fieldClass.length === 0){
            return;
        }
        let sizeClass =  classArr.filter(clas =>  clas.startsWith('ae-featured-img-size-'));
        const fieldName = fieldClass[0].replace('ae-feature-bg-custom-field-', '');
        
        const imageSize = sizeClass[0].replace('ae-featured-img-size-', '');
        var data = {
            fieldName : fieldName,
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                if(res.data){
                    $scope.css('background-image', 'url(' + res.data[0] + ')');
                }
